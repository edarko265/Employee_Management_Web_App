...
const AdminSettings = () => {
  ...
  toast({ title: t('profile_updated'), description: t('profile_saved_success') });
  };

  const handleNotificationSave = async () => {
    await api.patch("/admin/settings/notifications", notifications);
  toast({ title: t('notifications_updated'), description: t('notifications_saved') });
  };

  const handleThemeSave = async () => {
    await api.patch("/admin/settings/theme", themeSettings);
  toast({ title: t('theme_updated'), description: t('theme_saved') });
  };

  const handlePasswordUpdate = async () => {
    if (!profileData.newPassword || profileData.newPassword !== profileData.confirmPassword) {
    toast({ title: t('password_error'), description: t('passwords_do_not_match'), variant: "destructive" });
      return;
    }
  ...
  toast({ title: t('password_updated'), description: t('password_changed') });
    setProfileData(prev => ({
      ...prev,
      currentPassword: "",
      newPassword: "",
            ...
            <CardContent className="p-8">
              <div className="flex items-center gap-6">
                ...
                <div className="flex-1">
                  ...
                  <Badge className={`mt-2 ${getRoleColor(profileData.role)}`}>
                    ...
                    {[
                      {
                        ...
                        desc: t('emailNotifications_desc'),
                        key: "emailNotifications",
                      },
                      {
                        label: t('push_notifications'),
                        desc: t('pushNotifications_desc'),
                        key: "pushNotifications",
                      },
                      {
                        label: t('task_reminders'),
                        desc: t('taskReminders_desc'),
                        key: "taskReminders",
                      },
                      {
                        label: t('weekly_reports'),
                        desc: t('weeklyReports_desc'),
                        key: "weeklyReports",
                      },
                      {
                        label: t('system_updates'),
                        desc: t('systemUpdates_desc'),
                        key: "systemUpdates",
                      },
                    ].map((item, idx) => (
