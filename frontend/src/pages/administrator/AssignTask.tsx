...
const AdminAssignTask = () => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get('/admin/recent-tasks'),
      api.get('/admin/workplaces'),
    ])
      ...
      <div className="flex-1 p-6 md:p-10">
        ...
        <div className="space-y-8 max-w-6xl mx-auto py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <Card className="lg:col-span-2 border-0 rounded-2xl bg-gradient-to-tr from-white to-[#f3e8ff] shadow-md">
              <CardHeader>
              ...
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select value={workplace} onValueChange={setWorkplace}>
                      <SelectTrigger className="bg-white border rounded-lg px-3 py-2 focus:ring-2 focus:ring-violet-300">
                      ...
                      <SelectContent>
                        {workplaces.map((wp) => (
                          <SelectItem 
                            key={wp.id} 
                            value={wp.id}
                            className="flex flex-col items-start px-3 py-2 hover:bg-violet-50 rounded-lg transition-colors"
                          >
                            <span className="font-medium">{wp.name}</span>
                            {/* <span className="text-xs text-muted-foreground">{wp.address}</span> */}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <Select value={selectedCleaner} onValueChange={setSelectedCleaner}>
                      ...
                      <SelectContent>
                        {cleaners.map((cleaner) => (
                          <SelectItem key={cleaner.id} value={cleaner.id} disabled={cleaner.status === 'Busy'}>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="hours">{t('estimated_hours')}</Label>
                    <Input
                      id="hours"
                      type="number"
                ...
                <Button onClick={handleAssignTask} className="w-full bg-[#39092c] hover:bg-[#4e1850] text-white">
